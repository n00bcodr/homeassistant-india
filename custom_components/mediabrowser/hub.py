...
class MediaBrowserHub:
    ...
    def __init__(self, options: dict[str, Any]) -> None:
        ...
        self.user_id: str | None = options.get(CONF_USER_ID) or options.get(CONF_CACHE_SERVER_USER_ID)
    ...
    async def _async_needs_authentication(self):
        if self.api_key is None:
            raise aiohttp.ClientResponseError(
                None, None, status=401, message="API key is required"
            )
            ...
            try:
                ...
                if err.status == 401:
                    raise aiohttp.ClientResponseError(
                        None, None, status=401, message="Invalid API key"
                    ) from err
                else:
                    raise err
