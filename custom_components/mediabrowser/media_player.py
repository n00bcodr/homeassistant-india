...
async def async_setup_entry(
    ...
    async def session_changed(
        ...
        if (
            ...
            and entry.options.get(CONF_PURGE_PLAYERS, False)
        ):
...
class MediaBrowserPlayer(MediaBrowserEntity, MediaPlayerEntity):
    ...
    def device_info(self) -> DeviceInfo | None:
        # Use the main server device instead of creating individual devices for each player
        return None
    ...
    async def async_send_message(self, text: str, header: str, timeout: int | None = None) -> None:
        ...
        if self._session is not None:
            data = {"Text": text, "Header": header}
            if timeout is not None:
                data["TimeoutMs"] = timeout * 1000
            await self.hub.async_command(self._session_key, "DisplayMessage", data)
    ...
    async def async_send_command(self, command: str, arguments: dict[str, Any] | None = None) -> None:
        ...
        if self._session is not None:
            await self.hub.async_command(self._session_key, command, arguments)