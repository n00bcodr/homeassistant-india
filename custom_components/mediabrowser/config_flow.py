...
class MediaBrowserConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_select(
        ...
        server_list = (
            {
                server[Discovery.ID]: f'{server[Discovery.NAME] or "Unknown"} ({server[Discovery.ADDRESS]})'
    ...
    async def async_step_manual(
        ...
        """Handle the manual configuration step."""
        errors: dict[str, str] = {}
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_USER_ID,
                    default=previous_input.get(CONF_USER_ID, ""),
                ): str,
                vol.Optional(
                    CONF_NAME,
...
class MediaBrowserOptionsFlow(OptionsFlow):
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_USER_ID,
                        default=previous_input.get(
                            CONF_USER_ID, self.options.get(CONF_USER_ID, "")
                        ),
                    ): str,
                }
            ),
    ...
    async def async_step_events(self, user_input: dict[str, Any] | None = None) -> FlowResult:
    ...
    async def async_step_players(self, user_input: dict[str, Any] | None = None) -> FlowResult:
    ...
    async def async_step_add_sensor(
        ...
        """Handle a step to add a new sensor."""
        if user_input:
...
async def _validate_config(
    ...
    save_user_id = options.get(CONF_CACHE_SERVER_USER_ID)

    if url is not None:
        options[CONF_URL] = url
    if api_key is not None:
        options[CONF_API_KEY] = api_key
        options[CONF_CACHE_SERVER_API_KEY] = api_key
    if user_id is not None:
        options[CONF_USER_ID] = user_id
    ...
    try:
        ...
        options[CONF_USER_ID] = user_id or save_user_id
